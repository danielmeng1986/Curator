# Curator v0.2 Album Path and Relation Migration

> Documentation status: Historical
> Owner: Database
> Last verified: 2026-08-11

This completed one-time migration instruction is retained for provenance. It
does not describe the current active Workspace model or authorize writes to the
retired `workspace_album` table. Current schema navigation begins at
[Curator Database Model](../Curator_Database_Model.md); retirement semantics are
recorded in [Historical Workspace Album](Historical-Workspace-Album.md).

## Original migration objective

Implement the then-approved v0.2 schema. Do not change unrelated tables or data.
Work against `database/Curator.db` and make the migration safe, repeatable, and auditable.
