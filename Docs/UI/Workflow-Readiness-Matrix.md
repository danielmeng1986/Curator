...
Evidence command: `npm run test:ui-readiness`. Result: 13/13 required suites
passed.
...
| BT-067 AI Review translation cache | Admin requests missing Simplified-Chinese assistance on demand; cached translations survive refresh while English remains authoritative. | `#/ai-work-items/:uuid/review`; Admin | UI-038 | Browser never contacts DeepL; provider failure is local; hide/show and refresh preserve Review drafts and cached evidence. | Ready — contract and disposable fake-provider browser acceptance pass. |
...
| UI-011A–D | AI Collection Workspace product, Review state, UI, and browser acceptance. |
...
fixtures, continues the audit after individual failures, and reports each task,
Specification, Backend evidence, duration, and sanitized artifact location. Any
required failure still produces a non-zero final result.

UI-016 established the two-consecutive-clean-run rule and completed it without
skips. Backend workflow readiness and full regression also passed at that
milestone. Volatile individual Backend test totals remain in their generated
test output rather than this living design document. The smaller authenticated
smoke scenario remains part of, rather than a substitute for, the complete gate.
