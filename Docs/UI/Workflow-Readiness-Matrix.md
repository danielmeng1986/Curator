...
| Entity repository/service/API regression | Manage permanent Albums, Models, Studios, Statuses, Photos, and relationships. | `/albums`, `/models`, `/studios`, `/statuses`; Reader/Writer | UI-005, UI-012, UI-024 | Validation, duplicate/self relation, reference-protected delete, Reader writes, interrupted drafts, and stale restore preserve prior state. | Not Implemented — CRUD evidence passes, but unsaved form/relationship edits and list navigation context are lost on refresh or route change without the UI-024 recovery contract. |
| BT-019 Import COPY, MOVE, database-only, preview; `test_import_workflow_acceptance` | Compose a resumable batch, review zero-write preview, and execute the selected filesystem/database mode with truthful results. | `/import/albums`; Writer | UI-006, UI-013, UI-025 | Invalid/collision/stale/cancelled/repeated/interrupted requests have no unintended mutation; partial failure matches durable/filesystem outcome. | Not Implemented — execution safety passes, but compose, reviewed selection, Preview, and result context are memory-only and reset on route render or refresh. |
...
| BT-041 Snapshot/Backup administration contract | Inspect, create, and clean Backend-controlled recovery points. | `/admin/backups`; Admin | UI-010B/D, UI-026 | Arbitrary paths, unauthorized/cancelled/dismissed cleanup, corruption, and partial failure never claim full success. | Not Implemented — Backend safety and explicit cancellation pass, but implicit Preview dismissal plus refresh/restart recovery is unspecified and unverified. |
| BT-042 protected database Restore orchestration | Restore a verified database recovery point after protective Snapshot and typed confirmation. | `/admin/restore`; Admin | UI-010C/D, UI-026 | Invalid/stale/duplicate/cancelled/dismissed Restore does not change the database; failure remains recoverable. | Not Implemented — Restore execution is protected, but its reviewed modal lacks the shared explicit-abandonment and interruption contract. |
...
The 2026-08-13 Specification audit supersedes that earlier UI classification:
ten suites passed when run, while permission disclosure failed because its
field-name assertion does not distinguish managed Registration Proof metadata
from plaintext secret material. UI-023 owns that gate correction. UI-024–027
own the interaction gaps, and UI-028 must produce two new complete clean runs
before affected rows return to Ready.
