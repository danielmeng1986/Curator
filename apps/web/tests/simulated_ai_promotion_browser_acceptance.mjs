...
try{
  ...
  await page.getByRole('button',{name:'Review Promotion'}).click();await page.getByLabel('Type the exact selected name').fill(chosenName);await page.getByRole('button',{name:'Promote reviewed name'}).click();await page.getByText('Album name Promotion completed.').waitFor();
