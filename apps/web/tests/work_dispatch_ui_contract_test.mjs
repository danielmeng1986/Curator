...
assert.match(pageSource, />Retry Writer<\/button>/);
...
assert.match(pageSource, />Re-run From Vision<\/button>/);
assert.match(pageSource, /\/ai-work-items\/\$\{encodeURIComponent\(uuid\)\}\/regenerate-vision/);
...
assert.match(failedRows,/>Retry Writer<\/button>/);assert.match(failedRows,/retryItem\('item-1',3,this\)/);
assert.match(failedRows,/>Cancel<\/button>/);assert.match(failedRows,/cancelItem\('item-1',3,this\)/);
const writerFailedRows=page._itemRows([{item_uuid:'item-3',version:4,run_state:'Failed',result_state:'AwaitingWriter',attempt_count:2,configuration_snapshot:{name:'Balanced',model_file:'qwen.gguf'}}],'Fixture Album');
assert.match(writerFailedRows,/>Re-run From Vision<\/button>/);assert.match(writerFailedRows,/regenerateVision\('item-3',4,this\)/);
