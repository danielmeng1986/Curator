...
try{
  ...
  await page.getByRole('button',{name:'Review Promotion'}).click();await page.getByRole('heading',{name:'Confirm Album Name Promotion'}).waitFor();await page.getByLabel('Type the exact selected name').fill('Golden Forest Morning');await page.getByRole('button',{name:'Promote reviewed name'}).click();await page.getByText('Album name Promotion completed.').waitFor();
