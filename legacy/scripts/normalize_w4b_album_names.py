#!/usr/bin/env python3
"""Normalize W4B album names in workspace_album.

Rules:
- Only rows where studio_name = 'W4B' are processed.
- Only transform the album_name text by converting it to title case.
- Preserve the original text structure, only adjust capitalization.
...
def normalize_album_name(album_name: Optional[str]) -> Optional[str]:
    ...
    normalized_album = title_case_words(album_name)
    return normalized_album if normalized_album else album_name
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Normalize W4B album names in workspace_album.")
